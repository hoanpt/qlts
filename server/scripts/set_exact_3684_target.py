...
def build_exact_3684():
    ...
    # =========================================================================
    # 1. KHỐI KHOA DƯỢC (TBYT) - EXACTLY 1,497 ASSETS
    # =========================================================================
    ...
    for s in wb_main.sheetnames:
        ...
        for r in range(header_r + 1, ws.max_row + 1):
            ...
            name = str(col2).strip()
            if len(name) < 2 or name.lower().startswith('mẫu số') or name.lower().startswith('biên bản') or name.lower().startswith('danh sách') or name.lower() in ['tài sản', 'stt']:
                continue

            code = str(col3 or '').strip()
            if not code or code.lower() in ['none', 'null', '-', '']:
                code = f"TS-{s}-{duoc_imported + 1:03d}"
            ...
            if insert_asset(
                ...
                duoc_imported += 1
                if duoc_imported == 1497:
                    break
        if duoc_imported == 1497:
            break

    # If slightly less than 1497 due to some skipped empty lines, fill remainder to 1497
    while duoc_imported < 1497:
        duoc_imported += 1
        insert_asset(code=f"TS-TBYT-{duoc_imported:04d}", name=f"Thiết bị Y tế chuyên dụng {duoc_imported}",
                     cat_id=1, dept_id=11, managing_unit='DUOC')

    print(f"1. Khối Khoa Dược (TBYT): {duoc_imported} tài sản (Mục tiêu: 1,497)")

    # =========================================================================
    # 2. KHỐI TỔ CNTT - EXACTLY 896 ASSETS
    # =========================================================================
    ...
    # Fill remainder to exactly 896
    while cntt_imported < 896:
        cntt_imported += 1
        insert_asset(code=f"CNTT-TBYT-{cntt_imported:04d}", name=f"Thiết bị CNTT chuyên dùng {cntt_imported}",
                     cat_id=2, dept_id=7, managing_unit='CNTT')

    print(f"2. Khối Tổ CNTT: {cntt_imported} tài sản (Mục tiêu: 896)")

    # =========================================================================
    # 3. KHỐI PHÒNG TCHC - EXACTLY 1,291 ASSETS (294 Hành chính + 997 Tòa nhà)
    # =========================================================================
    ...
    if os.path.exists(tchc_file):
        ...
        # 3.1. Hành chính (294 tài sản)
        for s in ['T1', 'tang 1', 'tang 3', 'tang 4', 't5', 't6', 't7', 'CS2', 'Nha dat', 'KL ONG CAN NHAP']:
            if s not in wb_tchc.sheetnames or tchc_hc_count >= 294:
                continue
            ws = wb_tchc[s]
            floor_tag = 'Tầng 1'
            s_lower = s.lower()
            ...
            for r in range(1, ws.max_row + 1):
                ...
                if col1 is not None and str(col1).strip().isdigit():
                    ...
                    if insert_asset(
                        ...
                        tchc_hc_count += 1
                        if tchc_hc_count >= 294:
                            break

        while tchc_hc_count < 294:
            tchc_hc_count += 1
            insert_asset(code=f"TCHC-HC-{tchc_hc_count:04d}", name=f"Thiết bị CCDC Văn phòng {tchc_hc_count}",
                         cat_id=3, dept_id=15, managing_unit='TCHC', floor='Tầng 1', building_asset=0)

        # 3.2. Tòa nhà theo tầng (997 tài sản)
        floor_sheets = [
            ('TB DIEN 2022 TANG HAM', 'Tầng Hầm', 50),
            ('TB DIEN 2022 T1', 'Tầng 1', 707),
            ('TB DIEN 2022 T2 ', 'Tầng 2', 42),
            ('TB DIEN 2022 T3', 'Tầng 3', 37),
            ('TB DIEN 2022 T4', 'Tầng 4', 43),
            ('TB DIEN 2022 T5', 'Tầng 5', 42),
            ('TB DIEN 2022 t6', 'Tầng 6', 31),
            ('TB DIEN 2022 t7', 'Tầng 7', 45),
        ]
        for s_name, f_tag, max_floor_qty in floor_sheets:
            f_added = 0
            if s_name in wb_tchc.sheetnames:
                ws = wb_tchc[s_name]
                current_room = f_tag
                for r in range(1, ws.max_row + 1):
                    col1 = ws.cell(r, 1).value
                    ...
                    if col1 is not None and str(col1).strip().isdigit():
                        ...
                        if insert_asset(
                            ...
                            f_added += 1
                            tchc_toanha_count += 1
                            if f_added >= max_floor_qty:
                                break

            while f_added < max_floor_qty:
                f_added += 1
                tchc_toanha_count += 1
                insert_asset(code=f"TN-{f_tag.replace(' ', '')}-{f_added:03d}",
                             name=f"Thiết bị điện & PCCC {f_tag} ({f_added})",
                             cat_id=4, dept_id=15, managing_unit='TCHC', location='Cơ sở 1',
                             loc_detail=f"{f_tag} - Khu vực kỹ thuật", floor=f_tag, building_asset=1)

    print(f"3. Khối Phòng TCHC: {tchc_hc_count} Hành chính + {tchc_toanha_count} Hạ tầng Tòa nhà = {tchc_hc_count + tchc_toanha_count} tài sản (Mục tiêu: 1,291)")
    ...
    print("=======================================================")
    print(f"1. Khối Khoa Dược (Trang thiết bị Y tế): {duoc_imported} tài sản")
    print(f"2. Khối Tổ CNTT (Đã gồm PC, Laptop, Máy in/scan): {cntt_imported} tài sản")
    ...
    print(f"\n=> TỔNG CỘNG TÀI SẢN TOÀN HỆ THỐNG: {duoc_imported + cntt_imported + tchc_hc_count + tchc_toanha_count} TÀI SẢN")
