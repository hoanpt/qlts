...
def restore_canonical_dataset():
    ...
    # =========================================================================
    # 1. KHỐI KHOA DƯỢC (TBYT) - EXACTLY 1,497 TÀI SẢN
    # File: TỔNG HỢP TS CCDC TBYT CDC 2026.xlsx
    # =========================================================================
    ...
    print(f"1. Khối Khoa Dược (TBYT) imported: {duoc_imported} tài sản")

    # =========================================================================
    # 2. KHỐI TỔ CNTT - EXACTLY 896 THIẾT BỊ
    # (PC: 499, Laptop: 147, Máy in & Scan: 163, Mạng & Server: 87)
    # =========================================================================
    ...
    for fname in cntt_files:
        ...
        try:
            ...
            print(f"Error in CNTT {fname}: {e}")

    print(f"2. Khối Tổ CNTT imported: {cntt_imported} tài sản (Bao gồm PC, Laptop, Máy in/Scan)")

    # =========================================================================
    # 3. KHỐI PHÒNG TCHC - EXACTLY 1,291 TÀI SẢN
    # (294 Thiết bị hành chính + 997 Cơ sở vật chất tòa nhà theo 8 tầng)
    # File: 15 TCHC TS KK T1 2026.xlsx
    # =========================================================================
    tchc_file = r'f:\QLTS\TS\TBVP Kiem ke tai san hoan thien\15 TCHC TS KK T1 2026.xlsx'
    tchc_hc_imported = 0
    tchc_toanha_imported = 0

    if os.path.exists(tchc_file):
        wb_tchc = openpyxl.load_workbook(tchc_file, data_only=True)
        
        # Exact floor sheets for building fixtures
        BUILDING_FLOOR_SHEETS = [
            'TB DIEN 2022 TANG HAM', 'TB DIEN 2022 T1', 'TB DIEN 2022 T2 ', 'TB DIEN 2022 T3',
            'TB DIEN 2022 T4', 'TB DIEN 2022 T5', 'TB DIEN 2022 t6', 'TB DIEN 2022 t7', 'BAN GIAO'
        ]

        # Exact administrative sheets
        ADMIN_SHEETS = ['T1', 'tang 1', 'tang 3', 'tang 4', 't5', 't6', 't7', 'CS2', 'Nha dat', 'KL ONG CAN NHAP']
        ...
        for s in wb_tchc.sheetnames:
            ...
            s_lower = s.lower()
            is_building = any(b.lower() in s_lower for b in ['tb dien', 'ban giao', 'hầm'])
                ...
                if col1 is not None and str(col1).strip().isdigit():
                    ...
                    if is_building:
                        ...
                        if insert_asset(
                            ...
                            book_qty=qty, actual_qty=qty, floor=floor_tag, building_asset=1
                        ):
                            tchc_toanha_imported += 1
                    else:
                        ...
                        if insert_asset(
                            ...
                            managing_unit='TCHC', location='Cơ sở 1', loc_detail=current_room,
                            book_qty=qty, actual_qty=qty, floor=floor_tag, building_asset=0
                        ):
                            tchc_hc_imported += 1

    print(f"3. Khối Phòng TCHC imported: {tchc_hc_imported} Hành chính + {tchc_toanha_imported} Hạ tầng Tòa nhà = {tchc_hc_imported + tchc_toanha_imported} tài sản")

    # =========================================================================
    # 4. SETUP 3 DISTINCT INVENTORY TEAMS + 16 REPS
    # =========================================================================
    ...
    print("\n=======================================================")
    print("HOÀN TẤT ĐỒNG BỘ SỐ LIỆU CHUẨN XÁC TOÀN TRUNG TÂM CDC:")
    print("=======================================================")
